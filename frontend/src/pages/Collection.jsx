        ...
        <div className='w-[90%] md:w-[80%] m-auto flex flex-col sm:flex-row justify-between items-center gap-4 py-24 md:py-32'>
          ...
          <div className='flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto'>
            ...
            <div className={`relative transition-all duration-300 ease-in-out w-full sm:w-auto ${showSearch ? 'opacity-100' : 'opacity-0 sm:opacity-100'}`}>
              <input ref={searchInputRef} type="text" value={localSearch} onChange={handleSearch} placeholder="Search products..." className="w-full sm:w-64 bg-transparent border-b border-black text-black placeholder-gray-400 focus:outline-none focus:border-neutral-600 transition-colors duration-200 py-2" />

              <button onClick={handleSearchClose} className="absolute right-0 top-1/2 -translate-y-1/2 text-black hover:text-neutral-600 sm:hidden" aria-label="Close search" >
                <i className="ri-close-line"></i>
              </button>
            </div>

            <div className='relative'>
              <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className='flex items-center gap-2 text-black px-4 py-2 hover:text-neutral-600 transition-colors'>
                <span>SORT BY</span>
              ...
              <div className={`absolute right-0 mt-2 w-48 bg-white shadow-lg transform transition-all duration-300 origin-top-right z-20 ${isDropdownOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
                <ul className="py-2">
                  {['relevant', 'low-high', 'high-low'].map((type) => (
                    <li key={type}>
                      <button onClick={() => handleSortChange(type)} className="w-full px-4 py-3 text-left text-black hover:text-neutral-600 flex items-center gap-3">
            ...
            <button onClick={() => setIsFilterOpen(true)} className='flex items-center gap-2 text-black px-4 py-2 hover:text-neutral-600 transition-colors' >
              <span>FILTER</span>
